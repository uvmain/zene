      ...
      <div class="w-300 flex flex-col gap-4 p-6">
        <form class="flex flex-col gap-2">
          <div class="flex flex-row items-center gap-2">
            ...
            <input id="username" v-model="username" type="text" name="username" autocomplete="username" class="border rounded px-2 py-1">
          </div>
          <div class="flex flex-row items-center gap-2">
            <label for="password">Password:</label>
            <input id="password" v-model="password" type="password" name="password" autocomplete="current-password" class="border rounded px-2 py-1" @keydown.enter="login(username, password)">
          </div>
        </form>
      </div>
      <div class="flex justify-center gap-4">
        <button aria-label="login" class="px-4 py-2" @click="login(username, password)">
          Login
        </button>
      </div>
    </div>

...
<template>
  <div v-if="props.size === 'lg'" class="h-32 w-24 md:h-40 md:w-30">
    <img class="size-24 cursor-pointer rounded-lg object-cover md:size-30" :src="album.image_url" alt="Album Cover" @error="onImageError" @click="navigateAlbum()" />
  ...
  <div v-else-if="props.size === 'xl'" class="h-full flex flex-col items-center gap-4 from-zene-600/90 via-zene-600/80 bg-gradient-to-r p-4 md:flex-row md:gap-6 md:p-10">
    <img :src="album.image_url" class="size-32 cursor-pointer rounded-lg object-cover md:size-50" @error="onImageError" @click="navigateAlbum()">

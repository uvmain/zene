...
const { clearQueue, currentlyPlayingTrack, resetCurrentlyPlayingTrack, getNextTrack, getPreviousTrack, currentQueue, setCurrentQueue } = usePlaybackQueue()
...
<template>
  <div class="mx-auto max-w-4xl p-6 space-y-8">
    ...
    <!-- Current Track Info -->
    <div v-if="currentlyPlayingTrack" class="rounded-lg bg-zene-600 p-4">
      <h2 class="mb-2 text-xl text-white font-semibold">
        Currently Playing
      </h2>
      <div class="flex items-center space-x-4">
        <img v-if="currentlyPlayingTrack.image_url" :src="currentlyPlayingTrack.image_url" class="h-16 w-16 rounded-md" alt="Album art" />
        <div>
    ...
    <!-- Casting Status -->
    <div class="rounded-lg bg-zene-600 p-4">
      <h2 class="mb-2 text-xl text-white font-semibold">
        Casting Status
      </h2>
      <div class="flex items-center space-x-4">
        ...
        <button
          class="hover:bg-zene-300 rounded-md bg-zene-400 px-4 py-2 text-white transition-colors"
          @click="toggleCasting()"
        >
    ...
    <!-- Custom Seek Bar -->
    <div class="rounded-lg bg-zene-600 p-4">
      <h2 class="mb-4 text-xl text-white font-semibold">
        Seek Control
      </h2>
      <div v-if="audioRef || isCasting" class="flex flex-row items-center gap-4">
        ...
        <input
          type="range"
          class="h-2 flex-1 cursor-pointer rounded-lg bg-white/20 accent-zene-200"
    ...
    <div class="rounded-lg bg-zene-600 p-6">
      <h2 class="mb-4 text-xl text-white font-semibold">
        Media Controls
      </h2>
      <div class="flex flex-row items-center justify-center gap-x-6">
        <!-- Previous Button -->
        <button
          class="hover:bg-zene-300 h-12 w-12 flex cursor-pointer items-center justify-center rounded-md border-none bg-zene-400 text-white font-semibold outline-none transition-colors duration-200"

...
func HandleGetAlbumList(w http.ResponseWriter, r *http.Request) {
	...
	if typeParam == "genre" && genreParam == "" {
		net.WriteSubsonicError(w, r, types.ErrorMissingParameter, "genre parameter is required if type=genre", "")
		return
	}
	...
	logger.Printf("lintFixer: %d, %d, %d", version, sizeInt, offsetInt)

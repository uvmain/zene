...
func createFtsMetadataTable(ctx context.Context) {
	schema := `CREATE VIRTUAL TABLE metadata_fts
	...
	createVirtualTable(ctx, schema)
}

func createFtsMetadataTriggers(ctx context.Context) {
	createTrigger(ctx, `CREATE TRIGGER tr_metadata_insert_fts AFTER INSERT ON metadata
        BEGIN
	...
	createTrigger(ctx, `CREATE TRIGGER tr_metadata_delete_fts AFTER DELETE ON metadata
    BEGIN
        DELETE FROM metadata_fts WHERE file_path = old.file_path;
    END;`)

	createTrigger(ctx, `CREATE TRIGGER tr_metadata_update_fts AFTER UPDATE ON metadata
    BEGIN
...
func createFtsArtistsTable(ctx context.Context) {
	schema := `CREATE VIRTUAL TABLE artists_fts USING fts5(file_path, artist, tokenize="trigram remove_diacritics 1");`
	createVirtualTable(ctx, schema)
}

func createFtsArtistsTriggers(ctx context.Context) {
	createTrigger(ctx, `CREATE TRIGGER tr_metadata_insert_artists_fts AFTER INSERT ON metadata
        BEGIN
            INSERT INTO artists_fts (file_path, artist) VALUES (new.file_path, new.artist);
        END;`)

	createTrigger(ctx, `CREATE TRIGGER tr_metadata_delete_artists_fts AFTER DELETE ON metadata
    BEGIN
        DELETE FROM artists_fts WHERE file_path = old.file_path;
    END;`)

	createTrigger(ctx, `CREATE TRIGGER tr_metadata_update_artists_fts AFTER UPDATE ON metadata
    BEGIN
